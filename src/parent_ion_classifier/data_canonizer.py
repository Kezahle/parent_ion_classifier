...
class DataCanonizer:
    """
    Preprocess and standardize mass spectrometry data for model input.
    ...
    Attributes:
        N: Maximum number of rows to keep (top-N by intensity)
        data_columns: Column names for feature data (e.g., ['mz', 'MS1', 'MS2'])
        label_column: Column name for labels (e.g., 'parent')
        missing_val_replacement: Value to use for missing data (default: -1)
        intensity_max: Maximum value for scaled intensities (default: 10)
        mz_decimal_digits: Decimal places to round m/z values (default: 2)
        mz_msb_digits: Most significant digits to keep in m/z (default: -1, disabled)
    ...
    def _validate_input(self, df: pd.DataFrame) -> None:
        ...
        assert all(
            col in df.columns for col in self.data_columns
    ...
    def _scale_df(self, df: pd.DataFrame, cols: List[str], reset_index: bool = False) -> None:
        """
    ...
    def canonise_and_truncate_df(
        self, in_df: pd.DataFrame, reset_index: bool = False, sort_by_MS2_first: bool = True
    ) -> pd.DataFrame:
        """
        ...
        if pad_rows > 0:
            padding = pd.DataFrame(
                {col: self.missing_val_replacement for col in truncated_df.columns},
    ...
    def df_to_tensors(self, df: pd.DataFrame) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        label = (
            torch.tensor(df[self.label_column].values, dtype=torch.float32).squeeze()
